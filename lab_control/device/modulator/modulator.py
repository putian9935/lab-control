...
class Modulator(Target):
    def __init__(self, *, device_addr, ts_channel, output_channel, **kwargs) -> None:
        ...
        self.update_amplitude = alru_cache()(self.update_amplitude)
        self.update_frequency = alru_cache()(self.update_frequency)
        self.update_duty_cycle = alru_cache()(self.update_duty_cycle)
...
class Modulator_AFG1022(Modulator):
    ...
    async def update_burst_ncycle(self, ncycle):   
        self.inst.write(f'SOURCE{self.output_channel}:BURS:SOUR')
...
class Modulator_RSDG5082(Modulator_AFG1022):
    ...
    async def update_duty_cycle(self, duty_cycle):
        self.inst.write(f'C{self.output_channel}:BSWV DUTY, {duty_cycle:.1f}')
