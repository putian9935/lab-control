...
class Experiment:
    ...
    def __call__(self, f) -> Any:
        async def ret(*args, **kwds):
            ...
            ret = f(*args, **kwds)
            print(f'[INFO] File parsed in {time.perf_counter()-tt} second(s)!')
            tt = time.perf_counter()
            await run_prerequisite() 
            print(f'[INFO] Prerequisite done in {time.perf_counter()-tt} second(s)!')
            if self.to_fpga:
                tt = time.perf_counter()
                exp_time = prepare_sequencer_files()
                print(f'[INFO] Sequence prepared in {time.perf_counter()-tt} second(s)!')
                print(f'[INFO] Experiment cycle time: {exp_time/1e6} second(s)') 
                await run_sequence(self.ts_fpga, exp_time)
                print(f'[INFO] Sequence done!')
            clean_up()
            return ret 
